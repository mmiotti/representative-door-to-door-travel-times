...
Obstacles = require("obstacles_mod") -- UMA: load custom obstacles file
...
function setup()
  ...
  -- NOTE: make sure lon/lat ranges and number of rows/columns for these two datasets are correct (further below)
  local raster_path_impedance = "/data/rasterized_maps/impedance_car_hour_offpeak.asc"
  local raster_path_elev = "/data/rasterized_maps/elev_raster_dhm25.asc"
  ...
  return {
    properties = {
      ...
      uma_base_speed_multiplier      = 0.83,
      uma_impedance_multiplier       = 1.0,  -- use 0.0 to turn off impedance map
      uma_traffic_signal_penalty     = 9.4,  -- 2 + 8.04 * 0.92 -> note: used in obstacles_mod
      uma_stop_sign_penalty          = 5.0,  -- heuristic, not informed by model
      uma_intersection_penalty_3     = 1.2,  --  2.42 * 0.50
      uma_intersection_penalty_4     = 4.9,  --  6.08 * 0.81
      uma_elev_penalty_gain          = 0.0,
      uma_elev_penalty_loss          = 0.0,
    },
...
function process_turn(profile, turn)
  ...
  if turn.number_of_roads == 3 then
    turn.duration = turn.duration + profile.properties.uma_intersection_penalty_3
  end
  if turn.number_of_roads >= 4 then
    turn.duration = turn.duration + profile.properties.uma_intersection_penalty_4
  end
